...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.active.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.actives.update", [$active->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="code">{{ trans('cruds.active.fields.code') }}</label>
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $active->code) }}" required>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('classification') ? 'is-invalid' : '' }}" type="text" name="classification" id="classification" value="{{ old('classification', $active->classification) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $active->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cost') ? 'is-invalid' : '' }}" type="text" name="cost" id="cost" value="{{ old('cost', $active->cost) }}">
                @if($errors->has('cost'))
                    <div class="invalid-feedback">
                        {{ $errors->first('cost') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ubications') ? 'is-invalid' : '' }}" name="ubications[]" id="ubications" multiple required>
                    @foreach($ubications as $id => $ubication)
                        <option value="{{ $id }}" {{ (in_array($id, old('ubications', [])) || $active->ubications->contains($id)) ? 'selected' : '' }}>{{ $ubication }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('observations') ? 'is-invalid' : '' }}" type="text" name="observations" id="observations" value="{{ old('observations', $active->observations) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('condition') ? 'is-invalid' : '' }}" type="text" name="condition" id="condition" value="{{ old('condition', $active->condition) }}">
