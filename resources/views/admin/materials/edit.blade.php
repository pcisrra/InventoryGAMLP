    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.materials.update", [$material->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $material->description) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unity') ? 'is-invalid' : '' }}" type="text" name="unity" id="unity" value="{{ old('unity', $material->unity) }}">
                @if($errors->has('unity'))
                    <div class="invalid-feedback">
                        {{ $errors->first('unity') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $material->quantity) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unitary_cost') ? 'is-invalid' : '' }}" type="number" name="unitary_cost" id="unitary_cost" value="{{ old('unitary_cost', $material->unitary_cost) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_cost') ? 'is-invalid' : '' }}" type="number" name="total_cost" id="total_cost" value="{{ old('total_cost', $material->total_cost) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('localization') ? 'is-invalid' : '' }}" name="localization_id" id="localization_id" required>
                    @foreach($localizations as $id => $localization)
                        <option value="{{ $id }}" {{ (old('localization_id') ? old('localization_id') : $material->localization->id ?? '') == $id ? 'selected' : '' }}>{{ $localization }}</option>
                    @endforeach
                </select>
