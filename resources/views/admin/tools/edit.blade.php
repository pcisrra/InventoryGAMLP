    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tools.update", [$tool->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $tool->code) }}" required>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $tool->description) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $tool->quantity) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_asignados') ? 'is-invalid' : '' }}" name="user_asignados[]" id="user_asignados" multiple>
                    @foreach($user_asignados as $id => $user_asignado)
                        <option value="{{ $id }}" {{ (in_array($id, old('user_asignados', [])) || $tool->user_asignados->contains($id)) ? 'selected' : '' }}>{{ $user_asignado }}</option>
                    @endforeach
                </select>
