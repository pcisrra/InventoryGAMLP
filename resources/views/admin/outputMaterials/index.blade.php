@extends('layouts.admin')
@section('content')
@can('output_material_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.output-materials.create') }}">
                SOLICITAR
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header"></div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-OutputMaterial">
                <thead>
                    <tr>
                        <th width="10">

                        </th>
                        <th>
                            {{ trans('cruds.outputMaterial.fields.observations') }}
                        </th>
                        <th>
                            {{ trans('cruds.outputMaterial.fields.date_sol') }}
                        </th>
                        <th>
                            {{ trans('cruds.outputMaterial.fields.ots') }}
                        </th>
                        <th>
                            {{ trans('cruds.outputMaterial.fields.output_unity') }}
                        </th>
                        <th>
                            {{ trans('cruds.outputMaterial.fields.output_quantity') }}
                        </th>
                        <th>
                            {{ trans('cruds.outputMaterial.fields.material') }}
                        </th>
                        <th>
                            {{ trans('cruds.outputMaterial.fields.user') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($outputMaterials as $key => $outputMaterial)
                        <tr data-entry-id="{{ $outputMaterial->id }}">
                            <td>

                            </td>
                            <td>
                                {{ $outputMaterial->observations ?? '' }}
                            </td>
                            <td>
                                {{ $outputMaterial->date_sol ?? '' }}
                            </td>
                            <td>
                                {{ $outputMaterial->ots ?? '' }}
                            </td>
                            <td>
                                {{ $outputMaterial->output_unity ?? '' }}
                            </td>
                            <td>
                                {{ $outputMaterial->output_quantity ?? '' }}
                            </td>
                            <td>
                                {{ $outputMaterial->material->code ?? '' }}
                            </td>
                            <td>
                                {{ $outputMaterial->user->name ?? '' }}
                            </td>
                            <td>
                                <a class="btn btn-xs btn-primary" href="{{ route('admin.materials.exitMat',['codigo_material' => $outputMaterial->material->code, 'cantidad' => $outputMaterial->output_quantity]) }}">
                                    ACEPTAR
                                </a>

                                @can('output_material_delete')
                                    <form action="{{ route('admin.output-materials.destroy', $outputMaterial->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="RECHAZAR">
                                    </form>
                                @endcan
                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('output_material_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.output-materials.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 25,
  });
  let table = $('.datatable-OutputMaterial:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection